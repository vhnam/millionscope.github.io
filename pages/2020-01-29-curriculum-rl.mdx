...
![](assets/images/2020-01-29-curriculum-rl/types-of-curriculum-2.png)
...
![](assets/images/2020-01-29-curriculum-rl/curriculum-by-transfer-learning.png)
...
They noticed that combined strategy always outperformed the naive curriculum and would generally (but not always) outperform the mix strategy — indicating that it is quite important to mix in easy tasks during training to _avoid forgetting_.
...
![](assets/images/2020-01-29-curriculum-rl/teacher-student-curriculum.png)
...
![](assets/images/2020-01-29-curriculum-rl/ALP-GMM-algorithm.png)
...
![](assets/images/2020-01-29-curriculum-rl/self-play-maze.png)
...
- In the _self-play episode_, Alice alters the state from $s_0$ to $s_t$ and then Bob is asked to return the environment to its original state $s_0$ to get an internal reward.
- In the _target task episode_, Bob receives an external reward if he visits the target flag.
...
$$
\begin{aligned} \pi^{*}(a_t\vert s_t, g) &= \arg\max_\pi \mathbb{E}_{g\sim p_g(.)} R^g(\pi) \\
\text{where }R^g(\pi) &= \mathbb{E}_\pi(.\mid s_t, g) \mathbf{1}[\exists t \in [1,\dots, T]: s_t \in S^g] \end{aligned}
$$
...
$$
\begin{aligned} \mathcal{L}_\text{LSGAN}(D) &= \frac{1}{2} \mathbb{E}_{g \sim p_\text{data}(g)} [ (D(g) - b)^2] + \frac{1}{2} \mathbb{E}_{z \sim p_z(z)} [ (D(G(z)) - a)^2] \\
\mathcal{L}_\text{LSGAN}(G) &= \frac{1}{2} \mathbb{E}_{z \sim p_z(z)} [ (D(G(z)) - c)^2] \end{aligned}
$$
...
$$
\begin{aligned} \mathcal{L}_\text{GoalGAN}(D) &= \frac{1}{2} \mathbb{E}_{g \sim p_\text{data}(g)} [ (D(g) - b)^2 + (1-y_g) (D(g) - a)^2] + \frac{1}{2} \mathbb{E}_{z \sim p_z(z)} [ (D(G(z)) - a)^2] \\
\mathcal{L}_\text{GoalGAN}(G) &= \frac{1}{2} \mathbb{E}_{z \sim p_z(z)} [ (D(G(z)) - c)^2] \end{aligned}
$$

![](assets/images/2020-01-29-curriculum-rl/generative-goal-learning-algorithm.png)
...
1.  Goal **validity**: The proposed goal should be achievable by an expert policy. The corresponding generative loss is designed to increase the likelihood of generating goals that the solver policy has achieved before (like in [HER](https://arxiv.org/abs/1707.01495)).
                ...
                \begin{align*} \mathcal{L}*\text{val} = \mathbb{E}*{\substack{ g \sim \text{ achieved by solver}, \\
                \xi \in \text{Uniform}(0, \delta), \\
                f \in \text{Uniform}(0, 1) }} \big[ -\log p(G^{-1}(g + \xi, f)) \big] \end{align*}$$
    ...
    \*/} - $$
 \begin{aligned} \mathcal{L}_\text{val} = \mathbb{E}_{\substack{ g \sim \text{ achieved by solver}, \\
\xi \in \text{Uniform}(0, \delta), \\
f \in \text{Uniform}(0, 1) }} \big[ -\log p(G^{-1}(g + \xi, f)) \big] \end{aligned}$$
...
2.  Goal **feasibility**: The proposed goal should be achievable by the current policy; that is, the level of difficulty should be appropriate.
            ...
            \begin{align*} \mathcal{L}*\text{feas} = \mathbb{E}*{\substack{ z \in \mathcal{N}(0, 1), \\
            f \in \text{Uniform}(0, 1) }} \big[ D(G(z, f)) - \sigma^{-1}(f)^2 \big] \end{align*}$$

        \*/} - $$

    \begin{aligned} \mathcal{L}_\text{feas} = \mathbb{E}_{\substack{ z \in \mathcal{N}(0, 1), \\
    f \in \text{Uniform}(0, 1) }} \big[ D(G(z, f)) - \sigma^{-1}(f)^2 \big] \end{aligned}$$
...
3.  Goal **coverage**: We should maximize the entropy of generated goals to encourage diverse goal and to improve the coverage over the goal space.
    ...
    \begin{aligned} \mathcal{L}_\text{cov} = \mathbb{E}_{\substack{ z \in \mathcal{N}(0, 1), \\
    f \in \text{Uniform}(0, 1) }} \big[ \log p(G(z, f)) \big] \end{aligned}$$
...
![](assets/images/2020-01-29-curriculum-rl/setter-judge-goal-generation.png)
...
![](assets/images/2020-01-29-curriculum-rl/CARML.png)
...
$$
\begin{aligned} I(\tau; z) &= \mathcal{H}(z) - \mathcal{H}(z \vert s_1,\dots, s_T) \\
...
&\approx \mathbb{E}_{z\sim q_\phi(z), s\sim \pi_\theta(s|z)} [\color{green}{\log q_\phi(s|z) - \log q_\phi(s)}] & \scriptstyle{\text{; assume learned marginal distr. matches policy.}} \end{aligned}
$$
...
$$
\begin{aligned} r_z(s) &= \lambda \log q_\phi(s|z) - \log q_\phi(s) \\
...
where $W^{(k)}_i$ is the weight matrix of the layer $i$ in the column $k$;
$U_i^{( k\:j )}, j < k$
are the weight matrices for projecting the layer $i-1$ of the column $j$ to the layer $i$ of column $k$ ($ j < k $). The above weights matrices should be learned. $f(.)$ is a non-linear activation function by choice.

![](assets/images/2020-01-29-curriculum-rl/progressive-networks.png)
...
![](assets/images/2020-01-29-curriculum-rl/mix-and-match.png)
