...
description: "Are Deep Neural Networks Dramatically Overfitted?"
author: Lilian Weng
date: 2017-06-21
---
...
A _two-part version_ of MDL principle states that: Let $\mathcal{H}^{(1)}, \mathcal{H}^{(2)}, \dots$ be a list of models that can explain the dataset $\mathcal{D}$. The best hypothesis among them should be the one that minimizes the sum:
...
$$
\mathbf{M}_\text{ReLU} = \max\{\mathbf{X}\mathbf{W} - \boldsymbol{b}, 0 \} = \begin{bmatrix} \boldsymbol{x}_1\mathbf{W} - \boldsymbol{b} \\
 \dots \\
\boldsymbol{x}_n\mathbf{W} - \boldsymbol{b} \\
\end{bmatrix} = [\boldsymbol{x}_i\boldsymbol{W}_{(:,j)} - b_j]_{i \times j}
$$
...
![](assets/images/2019-03-14-overfit/nn-expressivity-proof.png)
...
$$
\mathbf{M}_\text{ReLU} = [a_i - b_j]_{i \times j} = \begin{bmatrix} a_1 - b_1 & 0 & 0 & \dots & 0 \\
\vdots & \ddots & & & \vdots \\
...
![](assets/images/2019-03-14-overfit/fit-random-labels.png)

Fig. 1. Fit models on CIFAR10 with random labels or random pixels: (a) learning curves; (b-c) label corruption ratio is the percentage of randomly shuffled labels. (Image source: [Zhang et al. 2017](https://arxiv.org/abs/1611.03530))
...
![](assets/images/2019-03-14-overfit/bias-variance-risk-curve.png)
...
![](assets/images/2019-03-14-overfit/new-bias-variance-risk-curve.png)
...
- The number of parameters is not a good measure of _inductive bias_, defined as the set of assumptions of a learning algorithm used to predict for unknown samples. See more discussion on DL model complexity in [later](#intrinsic-dimension) [sections](#heterogeneous-layer-robustness).
...
![](assets/images/2019-03-14-overfit/new-risk-curve-mnist.png)
...
![](assets/images/2019-03-14-overfit/regularization-generalization-test.png)
...
![](assets/images/2019-03-14-overfit/optimization-landscape-shape.png)
...
![](assets/images/2019-03-14-overfit/intrinsic-dimension-illustration.png)
...
By searching through the value of $d = 1, 2, \dots, D$, the corresponding $d$ when the solution emerges is defined as the _intrinsic dimension_.
...
![](assets/images/2019-03-14-overfit/intrinsic-dimension.png)
...
- **Re-initialization**: Reset the parameters to the initial values, $\theta^{(\ell)}_t \leftarrow \theta^{(\ell)}_0$. The performance of a network in which layer $\ell$ was re-initialized is referred to as the _re-initialization robustness_ of layer $\ell$.
- **Re-randomization**: Re-sampling the layer's parameters at random, $\theta^{(\ell)}_t \leftarrow \tilde{\theta}^{(\ell)} \sim \mathcal{P}^{(\ell)}$. The corresponding network performance is called the _re-randomization robustness_ of layer $\ell$.
...
![](assets/images/2019-03-14-overfit/layer-robustness-results.png)
...
![](assets/images/2019-03-14-overfit/layer-robustness-resnet.png)
...
The x-axis is the number of model parameters: (28 _ 28 + 1) _ num. units + num. units \* 10, in logarithm.

![](assets/images/2019-03-14-overfit/risk_curve_loss-mse_sample-4000_epoch-500.png)
...
![](assets/images/2019-03-14-overfit/layer_equality_256x3.png)
...
![](assets/images/2019-03-14-overfit/layer_equality_128x4.png)
...
![](assets/images/2019-03-14-overfit/intrinsic-dimension-net-64-64-and-128.png)
