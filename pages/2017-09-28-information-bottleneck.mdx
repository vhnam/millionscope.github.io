...
$$
\begin{aligned} D_{KL}(p \| q) &= \sum_x p(x) \log \frac{p(x)}{q(x)} \\
...
$$
\begin{aligned} I(X;Y) &= D_{KL}[p(x,y) \| p(x)p(y)] \\
...
![](assets/images/2017-09-28-information-bottleneck/ib-dnn-structure.png)

Fig. 1. The structure of a deep neural network, which consists of the target label $Y$, input layer $X$, hidden layers $h_1, \dots, h_m$ and the final prediction $\hat{Y}$. (Image source: [Tishby and Zaslavsky, 2015](https://arxiv.org/pdf/1503.02406.pdf))

If we label the hidden layers of a DNN as $h_1, h_2, \dots, h_m$ as in Fig. 1, we can view each layer as one state of a Markov Chain: $ h*i \to h*{i+1}$. According to DPI, we would have:

$$
\begin{aligned} H(X) \geq I(X; h_1) \geq I(X; h_2) \geq \dots \geq I(X; h_m) \geq I(X; \hat{Y}) \\
I(X; Y) \geq I(h_1; Y) \geq I(h_2; Y) \geq \dots \geq I(h_m; Y) \geq I(\hat{Y}; Y) \end{aligned}
$$
...
A DNN has successive internal representations of $X$, a set of hidden layers $\{T_i\}$. The _information plane_ theorem characterizes each layer by its encoder and decoder information. The encoder is a representation of the input data $X$, while the decoder translates the information in the current layer to the target ouput $Y$.
...
![](assets/images/2017-09-28-information-bottleneck/ib-information-plane.png)

Fig. 2. The encoder vs decoder mutual information of DNN hidden layers of 50 experiments. Different layers are color-coders, with green being the layer right next to the input and the orange being the furthest. There are three snapshots, at the initial epoch, 400 epochs and 9000 epochs respectively.
...
![](assets/images/2017-09-28-information-bottleneck/ib-information-plane-merged.png)
...
![](assets/images/2017-09-28-information-bottleneck/ib-mean-variation.png)
...
![](assets/images/2017-09-28-information-bottleneck/ib-bound.png)
...
![](assets/images/2017-09-28-information-bottleneck/ib-layers.png)
...
![](assets/images/2017-09-28-information-bottleneck/ib-training-size.png)
