...
![](assets/images/2017-08-20-gan/KL_JS_divergence.png)
...
![](assets/images/2017-08-20-gan/GAN.png)
...
$$
\begin{aligned} \min_G \max_D L(D, G) & = \mathbb{E}_{x \sim p_{r}(x)} [\log D(x)] + \mathbb{E}_{z \sim p_z(z)} [\log(1 - D(G(z)))] \\
& = \mathbb{E}_{x \sim p_{r}(x)} [\log D(x)] + \mathbb{E}_{x \sim p_g(x)} [\log(1 - D(x)] \end{aligned}
$$
...
$$
\begin{aligned} f(\tilde{x}) & = A log\tilde{x} + B log(1-\tilde{x}) \\
\frac{d f(\tilde{x})}{d \tilde{x}} & = A \frac{1}{ln10} \frac{1}{\tilde{x}} - B \frac{1}{ln10} \frac{1}{1 - \tilde{x}} \\
...
$$
\begin{aligned} L(G, D^*) &= \int_x \bigg( p_{r}(x) \log(D^*(x)) + p_g (x) \log(1 - D^*(x)) \bigg) dx \\
...
$$
\begin{aligned} D_{JS}(p_{r} \| p_g) =& \frac{1}{2} D_{KL}(p_{r} || \frac{p_{r} + p_g}{2}) + \frac{1}{2} D_{KL}(p_{g} || \frac{p_{r} + p_g}{2}) \\
...
=& \frac{1}{2} \bigg( \log4 + L(G, D^*) \bigg) \end{aligned}
$$
...
![](assets/images/2017-08-20-gan/nash_equilibrium.png)
...
![](assets/images/2017-08-20-gan/low_dim_manifold.png)
...
![](assets/images/2017-08-20-gan/GAN_vanishing_gradient.png)
...
![](assets/images/2017-08-20-gan/mode_collapse.png)
...
For both models, add $ | \Theta - \frac{1}{t} \sum\_{i=1}^t \Theta_i |^2 $ into the loss function, where $\Theta$ is the model parameter and $\Theta_i$ is how the parameter is configured at the past training time $i$. This addition piece penalizes the training speed when $\Theta$ is changing too dramatically in time.
...
\delta_2 &= 2 + 2 - 2 = 2\\
\delta_3 &= 2 + 1 - 4 = -1\\
\delta_4 &= -1 + 4 - 3 = 0 \end{aligned}
$$

Finally the Earth Mover's distance is $W = \sum \vert \delta_i \vert = 5$.

![](assets/images/2017-08-20-gan/EM_distance_discrete.png)
...
![](assets/images/2017-08-20-gan/wasserstein_simple_example.png)
...
$$
\begin{aligned} D_{KL}(P \| Q) &= \sum_{x=0, y \sim U(0, 1)} 1 \cdot \log\frac{1}{0} = +\infty \\
...
W(P, Q) &= |\theta| \end{aligned}
$$

But when $\theta = 0$, two distributions are fully overlapped:

$$
\begin{aligned} D_{KL}(P \| Q) &= D_{KL}(Q \| P) = D_{JS}(P, Q) = 0\\
W(P, Q) &= 0 = \lvert \theta \rvert \end{aligned}
$$
...
![](assets/images/2017-08-20-gan/WGAN_algorithm.png)
...
![](assets/images/2017-08-20-gan/pokemon-GAN.png)
