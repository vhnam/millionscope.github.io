...
$$
\begin{aligned} p(c | x_1, x_2, \dots, x_n) &\propto p(c, x_1, x_2, \dots, x_n)\\
&\propto p(c) p(x_1 | c) p(x_2 | c) \dots p(x_n | c)\\
&\propto p(c) \prod_{i=1}^n p(x_i | c). \end{aligned}
$$
...
Let's say we need to generate an explanation for a classification model $f: \mathbf{X} \rightarrow \mathbf{Y}$. Given a data point $x \in X$ which consists of $a$ individual values of attribute $A_i$, $i = 1, \dots, a$, and is labeled with class $y \in Y$. The _prediction difference_ is quantified by computing the difference between the model predicted probabilities with or without knowing $A_i$:
...
$$
\begin{aligned} p(y \vert x \backslash A_i) &= \sum_{s=1}^{m_i} p(A_i=a_s \vert x \backslash A_i) p(y \vert x \leftarrow A_i=a_s) \\
&\approx \sum_{s=1}^{m_i} p(A_i=a_s) p(y \vert x \leftarrow A_i=a_s) \end{aligned}
$$
...
![](assets/images/2017-08-01-interpretation/interpretability_prediction_decomposition.png)
...
The _local explanation vector_ is defined as the derivative of the probability prediction function at the test point $x = x_0$. A large entry in this vector highlights a feature with a big influence on the model decision; A positive sign indicates that increasing the feature would lower the probability of $x_0$ assigned to $f^{*}(x_0)$.
...
&\approx \frac{\sum_{i \in I_c} k_{\sigma} (x - x_i)}{\sum_i k_{\sigma} (x - x_i)} \end{aligned}
$$
...
![](assets/images/2017-08-01-interpretation/interpretability_local_gradient.png)
...
![](assets/images/2017-08-01-interpretation/LIME_interpretable_representation.png)
...
![](assets/images/2017-08-01-interpretation/LIME_illustration.png)
...
![](assets/images/2017-08-01-interpretation/LIME.png)
...
![](assets/images/2017-08-01-interpretation/BETA.png)
...
![](assets/images/2017-08-01-interpretation/adversarial_examples.png)
