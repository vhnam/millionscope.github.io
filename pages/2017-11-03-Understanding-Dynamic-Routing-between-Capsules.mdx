...
description: "A simple tutorial in understanding Capsules, Dynamic routing and Capsule Network CapsNet"
...
![fc](assets/images/capsule/fc.jpg)
...
![picasso](assets/images/capsule/picasso.jpg)
...
![face2](assets/images/capsule/face2.jpg)
...
![face4](assets/images/capsule/face4.jpg)
...
![face5](assets/images/capsule/face5.jpg)
...
![cnn1](assets/images/capsule/cnn1.jpg)

A capsule network share the same capsule to detect multiple variants in a simpler network.

![GM2](assets/images/capsule/c21.jpg)
![GM2](assets/images/capsule/c22.jpg)
...
![cap1](assets/images/capsule/cap1.jpg)
...
![style](assets/images/capsule/style.jpg)
...
![faa2](assets/images/capsule/faa2.jpg)
...
![fc2](assets/images/capsule/fc2.jpg)

We apply a **transformation matrix** $$ W*{ij} $$ to the capsule output $$ u*i $$ of the pervious layer. For example, with a $$ p \times k $$ matrix, we transform $$ u_i $$ to $$ \hat{u}_{j \vert i} $$ from k-dimension to p-dimension. ( $$ (p \times k) \times (k \times 1) \implies p \times 1 $$ ) Then we compute a weighted sum $$ s_j $$ with weights $$ c_{ij} $$ .
...
$$ c*{ij} $$ are **coupling coefficients** that are calculated by the iterative dynamic routing process (discussed next) and $$ \sum*{j} c*{ij} $$ are designed to sum to one. Conceptually, $$ c*{ij} $$ measures how likely capsule $$ i $$ may activate capsule $$ j $$ .
...
In capsule, we use iterative dynamic routing to compute the capsule output by calculating an intermediate value $$ c\_{ij} $$ (coupling coefficient).

![face6](assets/images/capsule/face6.jpg)
...
Therefore, the similarity score $$ b*{ij} $$ takes into account on both likeliness and the feature properties, instead of just likeliness in neurons. Also, $$ b*{ij} $$ remains low if the activation $$ u_i $$ of capsule $$ i $$ is low since $$ \hat{u}_{j \vert i} $$ length is proportional to $$ u_i $$ . i.e. $$ b_{ij} $$ should remain low between the mouth capsule and the face capsule if the mouth capsule is not activated.

The coupling coefficients $$ c*{ij} $$ is computed as the softmax of $$ b*{ij} $$ :
...
To make $$ b\_{ij} $$ more accurate , it is updated iteratively in multiple iterations (typically in 3 iterations).
...
![alg](assets/images/capsule/alg.jpg)
...
The dynamic routing is not a complete replacement of the backpropagation. The transformation matrix $$ W $$ is still trained with the backpropagation using a cost function. However, we use dynamic routing to compute the output of a capsule. We compute $$ c\_{ij} $$ to quantify the connection between a capsule and its parent capsules. This value is important but short lived. We re-initialize it to 0 for every datapoint before the dynamic routing calculation. To calculate a capsule output, training or testing, we always redo the dynamic routing calculation.
...
![over](assets/images/capsule/over.jpg)
...
![face7](assets/images/capsule/face7.jpg)
...
![arch1](assets/images/capsule/arch1.jpg)
...
It is then feed into DigiCaps which apply a transformation matrix $$ W\_{ij} $$ with shape 16x8 to convert the 8-D capsule to a 16-D capsule for each class $$ j $$ (from 1 to 10).
...
| DigiCaps        | Capsule output computed from a $$ W\_{ij} $$ (16x8 matrix) between $$ u_i $$ and $$ v_j $$ ( $$ i $$ from 1 to 32x6x6 and $$ j $$ from 1 to 10). | 10x16        |
...
_build_ declares the `self.W` parameters representing the transform matrix $W$ and `self.b` representing the $$ b\_{ij} $$ .
...
The code first expand the dimension of $ u_i $ and then multiple it with $ w $.
Nevertheless, the simple dot product implementation of $ W_{ij} u_i $ (commet out below) is replaced by `tf.scan` for better speed performance.
...
![dim](assets/images/capsule/dim.png)
