...
description: "Implementing Deep Reinforcement Learning Models with Tensorflow + OpenAI Gym"
author: Lilian Weng
date: 2018-05-05
---
...
_[*] For every new installation below, please make sure you are in the virtualenv._
...
![](assets/images/2018-05-05-drl-implementation/pacman-original.gif)
...
$$
\begin{aligned} & Y(s, a, r, s') = r + \gamma \max_{a'} Q_{\theta^{-}}(s', a') \\
& \mathcal{L}(\theta) = \mathbb{E}_{(s, a, r, s') \sim U(D)} \Big[ \big( Y(s, a, r, s') - Q_\theta(s, a) \big)^2 \Big] \end{aligned}
$$
...
![](assets/images/2018-05-05-drl-implementation/dqn-tensorboard-graph.png)
...
![](assets/images/2018-05-05-drl-implementation/tf_gather.png)
...
![](assets/images/2018-05-05-drl-implementation/dueling-q-network.png)
...
The policy network is contructed. We update the policy parameters by minimizing the loss function, $\mathcal{L} = - (G_t - V(s)) \log \pi(a \vert s)$. [tf.nn.sparse_softmax_cross_entropy_with_logits()](https://www.tensorflow.org/api_docs/python/tf/nn/sparse_softmax_cross_entropy_with_logits) asks for the raw logits as inputs, rather then the probabilities after softmax, and that's why we do not have a softmax layer on top of the policy network.
...
The tensorboard graph is always helpful: ![](assets/images/2018-05-05-drl-implementation/actor-critic-tensorboard-graph.png)
